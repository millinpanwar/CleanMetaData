...
import { MAX_SIZE, PDF_ACCEPT } from '../lib/validators.js'
...
export default function PdfMetadataRemover() {
  ...
  const inspect = async f => {
    ...
    form.append('file', f)
    const { data } = await api.post('/pdf/inspect', form, {
  ...
  const strip = async () => {
    ...
    const res = await api.post('/pdf/strip', form, { responseType: 'blob' })
    const url = URL.createObjectURL(res.data)
    const a = document.createElement('a')
    a.href = url
    a.download = (file.name.replace(/(\.[^.]*)$/, '')) + '-cleaned.pdf'
    a.click()
    URL.revokeObjectURL(url)
    setToast('Cleaned PDF downloaded. Server copy deleted.')
    setTimeout(()=>setToast(''), 3000)
  }

  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">PDF Metadata Remover</h1>
      <FileDropzone accept={PDF_ACCEPT} maxSizeBytes={MAX_SIZE} onFile={(f)=>{ if(f.size<=MAX_SIZE){ setFile(f); inspect(f).catch(console.error) } }}/>
