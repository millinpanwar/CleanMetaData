...
import { MAX_SIZE, VIDEO_ACCEPT } from '../lib/validators.js'
import FileDropzone from '../components/FileDropzone.jsx'
import ProgressBar from '../components/ProgressBar.jsx'
import Toast from '../components/Toast.jsx'

export default function RemoveVideoMetadata() {
  ...
  const inspect = async f => {
    ...
    form.append('file', f)
    const { data } = await api.post('/video/inspect', form, {
  ...
  const strip = async () => {
    ...
    const res = await api.post('/video/strip', form, {
      responseType: 'blob',
      onUploadProgress: e => setProgress(Math.round((e.loaded / e.total) * 100) || 0)
    })
    ...
    setToast('Cleaned video downloaded. Server copy deleted.')
    setTimeout(()=>setToast(''), 3000)
  }

  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">Remove Metadata from Video</h1>
      <FileDropzone accept={VIDEO_ACCEPT} maxSizeBytes={MAX_SIZE} onFile={(f)=>{ if(f.size<=MAX_SIZE){ setFile(f); inspect(f).catch(console.error) } }}/>
